...
public class AchievementServiceTest {
    ...
    public void addDaily() throws Exception {
        ...
        List<String> expectedList = Collections.singletonList(FIRST_ACHIEVEMENT_ID);
        ...
        List<String> actualList = service.addDaily(new DailyRequest("max", "Daily"));
    ...
    public void addSecondDaily() throws Exception {
        List<Achievement> userFromIdList = Collections.singletonList(
                new Achievement("max", "max", ONE_POINT, "Daily", AchievementType.DAILY));
        List<String> expectedList = Collections.singletonList(FIRST_ACHIEVEMENT_ID);
        ...
        List<String> actualList = service.addDaily(new DailyRequest("max", "Second daily"));
    ...
    public void addThanks() throws Exception {
        ...
        List<String> actualList = service.addThanks(new ThanksRequest("max", "john", "Thanks"));
    ...
    public void addSecondThanks() throws Exception {
        ...
        Achievement achievement = new Achievement("max", "john", ONE_POINT, "Thanks",
                AchievementType.THANKS);
        List<Achievement> userFromIdList = Collections.singletonList(achievement);
        when(repository.getAllAchievementsByUserFromIdCurrentDateType("max", AchievementType.THANKS))
                .thenReturn(userFromIdList);
        when(repository.addAchievement(any(Achievement.class))).thenReturn(SECOND_ACHIEVEMENT_ID)
                .thenReturn(THIRD_ACHIEVEMENT_ID);

        List<String> actualList = service
                .addThanks(new ThanksRequest("max", "bill", "Second thanks"));
    ...
    public void addThirdThanks() throws Exception {
        List<Achievement> userFromIdList = Arrays.asList(
                new Achievement("max", "john", ONE_POINT, "Thanks", AchievementType.THANKS),
                new Achievement("max", "bob", ONE_POINT, "Thanks", AchievementType.THANKS));

        when(repository.getAllAchievementsByUserFromIdCurrentDateType("max", AchievementType.THANKS))
                .thenReturn(userFromIdList);
        when(repository.addAchievement(any(Achievement.class))).thenThrow(ThanksAchievementMoreThanTwoException.class);

        service.addThanks(new ThanksRequest("max", "bill", "Third thanks"));
    }
    ...
    public void addSecondCodenjoy() throws Exception {
        List<Achievement> userFromIdList = Arrays.asList(
                new Achievement("max", "john", CODENJOY_FIRST_PLACE, "First place codenjoy",
                        AchievementType.CODENJOY),
                new Achievement("max", "bill", CODENJOY_SECOND_PLACE, "Second place codenjoy",
                        AchievementType.CODENJOY),
                new Achievement("max", "bob", CODENJOY_THIRD_PLACE, "Third place codenjoy",
                        AchievementType.CODENJOY));
    ...
    public void addInterview() throws Exception {
        ...
        List<String> actualList = service.addInterview(new InterviewRequest("max", "Interview"));
    ...
    public void addKeeperThanks() throws Exception {
        List<String> expectedList = Arrays.asList(FIRST_ACHIEVEMENT_ID, SECOND_ACHIEVEMENT_ID, THIRD_ACHIEVEMENT_ID);
        List<KeeperDTO> keepersList = new ArrayList<>(Arrays.asList(
                new KeeperDTO("0002A", Arrays.asList("First direction", "Second direction")),
                new KeeperDTO("0002B", Arrays.asList("Third direction"))));
