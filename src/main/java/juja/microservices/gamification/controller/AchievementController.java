...
public class AchievementController {
    ...
    public ResponseEntity<?> addDaily(@Valid @RequestBody DailyRequest request) {
        log.info("Received request on /achievements/daily from user: '{}', description: '{}'",
                request.getFrom(),
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    message = "First thanks per day returns array with " +
                            "one achievement id.\n" +
    ...
    public ResponseEntity<?> addThanks(@Valid @RequestBody ThanksRequest request) {
        log.info("Received request on /achievements/thanks from user: '{}', description: '{}'", request.getFrom(),
                request.getDescription());
    ...
    public ResponseEntity<?> addCodenjoy(@Valid @RequestBody CodenjoyRequest request) {
        log.info("Received request on /achievements/codenjoy from user: '{}', places: 1st-'{}', 2nd-'{}', 3rd-'{}'",
                request.getFrom(),
    ...
    public ResponseEntity<?> addInterview(@Valid @RequestBody InterviewRequest request) {
        log.info("Received request on /achievements/interview from user: '{}', description: '{}'", request.getFrom(), request.getDescription());
    ...
    public ResponseEntity<?> addThanksKeeper() {
        log.info("Received request on /achievements/keepers/thanks");
    ...
    @ApiOperation(value = "Add welcome points to new user", notes = "This method adds points to new user")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> addWelcome(@Valid @RequestBody WelcomeRequest request) {
        log.info("Received request on /achievements/welcome from user: '{}'", request.getFrom());
    ...
    public ResponseEntity<?> addTeam(@Valid @RequestBody TeamRequest request) {
        log.info("Received request on /achievements/team from user: '{}'", request.getFrom());
